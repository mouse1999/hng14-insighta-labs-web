...
export default function FiltersBar({ filters, onApply }: FiltersBarProps) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      // §7: api.exportCSV sends X-API-Version: 1 via fetch+blob
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 z-40 w-80 bg-ink-900 border border-white/[0.09] rounded-2xl p-5 shadow-2xl animate-fade-up">
          ...
          <div className="flex gap-2 mt-5 pt-4 border-t border-white/[0.06]">
            <button onClick={handleReset} className="flex-1 py-2 rounded-xl text-xs text-mist-dim hover:text-mist border border-white/[0.08] hover:bg-ink-800 transition-all">
              Reset
            </button>
            <button onClick={handleApply} className="flex-1 py-2 rounded-xl bg-acid hover:bg-acid-dim text-ink-950 text-xs font-display font-semibold transition-all">
              Apply
            </button>
          </div>
        </div>
