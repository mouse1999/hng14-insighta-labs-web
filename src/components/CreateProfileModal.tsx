...
export default function CreateProfileModal({ onClose, onCreated }: CreateProfileModalProps) {
  ...
  const validate = (val: string) => {
    ...
    if (!NAME_REGEX.test(val)) return 'Only letters A–Z allowed (no numbers or special characters)';
    return '';
  };
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              autoFocus
              className="w-full bg-ink-800 border border-white/[0.08] focus:border-acid/50 rounded-xl px-4 py-3 text-mist text-sm font-body placeholder:text-mist-dim/50 outline-none transition-all"
            />
            {error && (
            ...
            <p className="mt-2 text-[11px] text-mist-dim">
              Only letters (A–Z) are allowed. Our API predicts gender, age, and nationality automatically.
            </p>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-white/[0.08] text-mist-dim hover:text-mist hover:bg-ink-800 text-sm font-medium transition-all"
            >
              Cancel
            </button>
            <button
