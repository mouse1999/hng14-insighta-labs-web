...
export default function SearchPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (data) {
        setData({ ...data, data: data.data.filter((p: Profile) => p.id !== id) });
      }
    } catch (e) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-ink-900 border border-white/[0.08] focus:border-acid/40 rounded-2xl pl-11 pr-28 py-4 text-mist text-sm font-body placeholder:text-mist-dim/50 outline-none transition-all"
        />
        <button
          ...
          <div className="flex flex-wrap gap-2">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="px-3 py-1.5 bg-ink-900 border border-white/[0.07] hover:border-acid/30 hover:bg-acid/5 text-mist-dim hover:text-acid rounded-xl text-xs font-body transition-all"
              >
                {ex}
              </button>
            ))}
